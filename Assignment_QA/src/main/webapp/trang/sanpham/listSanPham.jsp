...
<div class="text-start mb-3 mt-4">
    ...
    <a href="showAddSP" class="btn btn-success">Thêm</a>
</div>
<table class="table table-bordered table-sm container text-center">
    ...
    <c:forEach items="${listSP}" var="u">
          <tr>
              <td><img src="img/sanpham/${u.anhSP}" width="200"></td>
              <td>${u.maSP}</td>
              <td>${u.tenSP}</td>
              <td>
                  <a href="/removeSP?id=${u.idSP}" class="btn btn-danger" onclick="return confirm('Bạn có muốn xóa không ?')" >Xóa</a>
                  <a href="/detailSP?id=${u.idSP}" class="btn btn-primary">Detail</a>
              </td>
          </tr>
    </c:forEach>
    </tbody>
